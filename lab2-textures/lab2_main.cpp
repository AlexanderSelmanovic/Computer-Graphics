...
GLuint vertexArrayObjectExplosion, exBuffer, exTexBuffer, exIndexBuffer;

GLuint texture, explosion;
...
void initGL()
{
	...
	glGenBuffers(1, &exBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, exBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(exPos), exPos, GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, false /*normalized*/, 0 /*stride*/, 0 /*offset*/);
	glEnableVertexAttribArray(0);
	...
	glGenBuffers(1, &exTexBuffer);
	glBindBuffer(GL_ARRAY_BUFFER, exTexBuffer);
	glBufferData(GL_ARRAY_BUFFER, sizeof(exCoord), exCoord, GL_STATIC_DRAW);
	glVertexAttribPointer(2, 2, GL_FLOAT, false/*normalized*/, 0/*stride*/, 0/*offset*/);
	glEnableVertexAttribArray(2);
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(exIndices), exIndices, GL_STATIC_DRAW);
	...
	int w2, h2, comp2;
	unsigned char* exImage = stbi_load("../scenes/explosion.png", &w2, &h2, &comp2, STBI_rgb_alpha);

	glGenTextures(1, &explosion);
	glBindTexture(GL_TEXTURE_2D, explosion);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w2, h2, 0, GL_RGBA, GL_UNSIGNED_BYTE, exImage);
	free(exImage);
...
void gui()
{
	...
	if (ImGui::RadioButton("GL_NEAREST", &mag, 0)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	}
	else if (ImGui::RadioButton("GL_LINEAR", &mag, 1)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	}
	ImGui::PopID();
	...
	if (ImGui::RadioButton("GL_NEAREST", &mini, 0)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	}
	else if (ImGui::RadioButton("GL_LINEAR", &mini, 1)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	}
	else if (ImGui::RadioButton("GL_NEAREST_MIPMAP_NEAREST", &mini, 2)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
	}
	else if (ImGui::RadioButton("GL_NEAREST_MIPMAP_LINEAR", &mini, 3)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR);
	}
	else if (ImGui::RadioButton("GL_LINEAR_MIPMAP_NEAREST", &mini, 4)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
	}
	else if (ImGui::RadioButton("GL_LINEAR_MIPMAP_LINEAR", &mini, 5)) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
	}
	
	ImGui::PopID();

	if (ImGui::SliderFloat("Anisotropic filtering", &anisotropy, 1.0, 16.0, "Number of samples: %.0f")) {
