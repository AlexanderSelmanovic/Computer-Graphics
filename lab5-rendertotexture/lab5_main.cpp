...
struct FboInfo
{
	...
	FboInfo(int w, int h)
	{
		...
		// bind the texture as color attachmen 0
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorTextureTarget, 0);
		glDrawBuffer(GL_COLOR_ATTACHMENT0);

		// bind the textures as depth attachment (to the currently bound framebuffer)
...
void display()
{
	...
	///////////////////////////////////////////////////////////////////////////
	FboInfo& mainCameraFB = fboList[1];
	glBindFramebuffer(GL_FRAMEBUFFER, mainCameraFB.framebufferId); // to be replaced with another framebuffer when doing post processing
	glViewport(0, 0, w, h);
	...
	glUseProgram(postFxShader); // set active shader
	...
	glBindTexture(GL_TEXTURE_2D, mainCameraFB.colorTextureTarget);
